/**
 * searchList — Deduplication logic (identical to searchListAsync)
 */
...
export function processElementsWithXPath(elementsList) {
  ...
  const areSameComponent = (x1, x2) => {
    const p1 = parseXPath(x1), p2 = parseXPath(x2);
    ...
    return p1.slice(0, commonLen).every((item, idx) => item.tagName === p2[idx].tagName && item.index === p2[idx].index);
  };
  ...
  elementsList.forEach((el) => {
    if (el.xPath && el.coordinates) {
      ...
      for (const [key, group] of grouped) {
        if (areSameComponent(el.xPath, key) && el.coordinates === group[0].coordinates) { group.push(el); found = true; break; }
      }
...
export function findUniqueAndOneRepeated(elementsList) {
  const wordFreq = new Map(), wordToItems = new Map(), coordsMap = new Map();
  ...
  const repeatedWords = Array.from(wordFreq.entries()).filter(([, c]) => c > 1).map(([w]) => w);
  const result = [], added = new Set();

  repeatedWords.forEach((word) => {
    if (wordToItems.has(word)) {
      const items = Array.from(wordToItems.get(word));
      items.sort((a, b) => hasAttr(b, 'aria-label') - hasAttr(a, 'aria-label') || hasAttr(b, 'test-id') - hasAttr(a, 'test-id'));
  ...
  const uniqueCoords = new Map(), filteredResult = [];
  result.forEach((el) => {
    if (el.coordinates) {
      if (!uniqueCoords.has(el.coordinates)) { uniqueCoords.set(el.coordinates, el); filteredResult.push(el); }
      else {
        ...
        if ((!hasAttr(existing, 'aria-label') && hasAttr(el, 'aria-label')) ||
          ...
          uniqueCoords.set(el.coordinates, el); filteredResult[filteredResult.indexOf(existing)] = el;
        }
      }
    } else filteredResult.push(el);
  });

  const uniqueXPaths = new Set(), finalResult = [];
  filteredResult.forEach((el) => { if (el.xPath && !uniqueXPaths.has(el.xPath)) { uniqueXPaths.add(el.xPath); finalResult.push(el); } });
