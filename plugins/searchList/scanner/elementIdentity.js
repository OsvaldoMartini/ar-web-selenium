/**
 * searchList — Element identity extraction
 */
...
export function extractVisibleTextFromHTML(element) {
  ...
  element.querySelectorAll('label').forEach((label) => {
    ...
    if (forAttr) {
      ...
      if (inp && isVisible(inp)) {
        ...
        if (raw) { const f = raw.split(/\s+/).filter((w) => !isTech(w)).join(' ').trim(); if (f) result.text.add(f); }
      }
    }
  });

  ['p','h1','h2','h3','h4','h5','h6','li','span','div','strong','em','b','i','blockquote'].forEach((tag) => {
  ...
  element.querySelectorAll('iframe').forEach((iframe) => {
    if (iframe.hasAttribute('title')) { const t = iframe.getAttribute('title')?.trim(); if (t) result.titles.add(t); }
    try {
    ...
    } catch (e) { console.log('Could not access iframe content', e); }
  });

  return { text: Array.from(result.text), labels: Array.from(result.labels), titles: Array.from(result.titles) };
}
...
function getVisibleText(tagName, attributeData, element) {
  ...
  if (element && !isHidden(element)) {
    ...
    textResult = [...extracted.titles, ...extracted.text, ...extracted.labels].map((t) => t.trim()).filter(Boolean).join('; ');
  }

  const priority = ['aria-label','aria-labelledby','aria-describedby','textarea','input','select','placeholder','label','name','title','alt','for','data-label','data-name','data-title','id','data-testid'];
  let first = '';
  ...
  if (textResult && !/^\..*\{.*\}$/.test(textResult)) { first = textResult; }
  else {
    const titleAttr = attributeData.find(({ name }) => name === 'title');
    if (titleAttr) first = getAttrText(titleAttr.name, titleAttr.value);
    if (!first) {
      for (const attr of priority) {
        const found = attributeData.find(({ name }) => name === attr);
        if (found) { first = getAttrText(found.name, found.value); if (first) break; }
      }
    }
  }
  return first;
