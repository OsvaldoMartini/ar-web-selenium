/**
 * searchList — DOM walker, element collection, shadow DOM push
 */
...
export function collectElements(doc, searchTerms, collectionFound) {
  if (searchTerms.length > 0) {
    ...
    collectionFound.push(...Array.from(doc.querySelectorAll('*')).filter((el) => el.tagName.toLowerCase() !== 'iframe'));
  }

  collectionFound.forEach((element) => {
    if (['html','body','main','script','meta','head','style'].includes(element.tagName.toLowerCase())) return;
    const id = getElementIdentity(window.__slSync_hiddenFields, element);
...
function resolveShadowAndPush(element, elementIdentity, referXPath, typeDTO) {
  ...
  if (shadowHost && shadowHost.shadowRoot) {
    ...
    findClickableElements(sr).forEach((el) => {
      pushElement(el, elementIdentity, referXPath, typeDTO, shadowHost, sr);
    });
  } else {
...
function pushElement(element, identityTemp, referXPath, typeDTO, shadowHost, shadowRoot) {
  ...
  if (shadowPath.length > 0) cssSelector = shadowPath.reduceRight((acc, host) => `${host} ${acc}`, elementCssSelector);
  ...
  if (identity) window.elementInfoMap.set(referXPath, elementDTO(typeDTO, identity));
}
