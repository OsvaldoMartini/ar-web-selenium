/**
 * searchListAsync — Tag classification from XPath
 *
 * Note: this version does NOT guard with isActuallyClickable
 * (matches the original script-search-in-use-list-async behaviour).
 */
...
export function classifyTag(tagName, xpath) {
  ...
  for (let i = parts.length - 1; i >= 0; i--) {
    ...
    if (tag.includes('expansion-panel-header') || tag.includes('sidenav') || tag.includes('nav'))
      return 'button';

    if (tag === 'select' || tag === 'option') return 'select';
    if (tag === 'textarea') return 'input';

    if (tag.includes('mat-button') || tag.includes('mat-raised-button') ||
        tag.includes('mat-icon-button') || tag.includes('mat-menu-item') ||
        tag.includes('mat-select') || tag.includes('mat-option') || tag.includes('matinput'))
      return 'button';

    if (tag.includes('data-testid') || tag.includes('aria-label') ||
        part.includes("@role='button'") || part.includes("@role='textbox'") ||
        part.includes('react-button') || part.includes('react-link') || part.includes('react-input')) {
