...
- [x] Frontend `9723c6e` adds the green/red `ON/OFF Continue Page` control in the Integration toolbar. ON continues an already-open same-site Java V1 Playwright page; OFF reloads the selected Bot Job URL. V2 forces OFF because its isolated runtime cannot adopt the shared Java browser.
- [x] Backend `b2d13bde` makes `PRESERVE_ACTIVE` and `RELOAD_SELECTED` authoritative, echoes the accepted policy, validates the current page against the selected Bot Job origin, and refuses cross-site reuse rather than contaminating another Bot Job.
- [x] Focused verification passed 11/11 across `SmokeTestIntegrationContractsTest` and `SmokeTestIntegrationServiceTest`. The production frontend build passed. No broad test suite or package build ran.
...
- [x] Matched runtime pair restarted: Node PID `4560` is READY on `60110`; ARWeb PID `24304` is responsive on `54921/54922`. Live JS/CSS hashes match `target/classes`, and six new logs contain zero strict Java/SQLite/snapshot errors.
- [ ] Live user gate: open Lloyds Job 29, reach the password step, stop/close/reopen ARWeb as desired, then run again with Continue Page ON and confirm the current authenticated same-origin page is preserved. Repeat once with OFF and confirm it reloads the selected Lloyds URL.
