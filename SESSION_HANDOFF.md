...
- [ ] The migration was created but not applied to BancaStato or any other database. Startup migrations remain parked; applying it requires an authorized maintenance window, exact backup, post-migration graph/count verification, and rollback evidence.
- [ ] No application restart or live ExcelWrite file write was performed in this checkpoint. After migration/restart, acceptance must configure two ExcelWrite instructions in different Blocks to the same file, execute values from their independent READ variables, verify both columns/file contents, verify an unconfigured instruction bypasses, and confirm variables survive instruction/Block deletion.
