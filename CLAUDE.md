...
## UI gating rules (standing order — from the 2026-07-31 Variables regression)

- **Never gate a user gesture silently.** Every disabled button, ignored drag, refused drop, or dropped
  server response MUST surface its reason to the user (chip, tooltip, status line, or alert). A silent
  `return null` in a submit/enable path is a bug by definition in this project.
- **Never land a large gating/authoring layer in one commit.** The 2026-07-31 incident: one +4,300-line
  FE commit made the Variables page appear fully "blocked" while every mutation was actually COMMITTING
  to the database (`instruction_graph_state` advanced ~50 versions invisibly). Re-land such features in
  small, individually runtime-accepted commits.
