...
public class ARControlPanel extends Application {
  ...
  private static String defaultConfigurationFileName =
      ARConstants.USER_PATH + ARConstants.FILE_NAME_CONFIGURATION;
  ...
  public static void main(String[] args) {
    ...
    if (arguments.contains("-c")) {
      ...
      ARLogger.getInstance(ARControlPanel.class)
          .fine("Configuration file path: " + configurationValue);
    } else {
      try {
      ...
      ARLogger.getInstance(ARControlPanel.class)
          .fine("Configuration file path: " + defaultConfigurationFileName);
    }
  ...
  private static void licenseControl() {
    ...
    if (isEnabledLicence) {
          ...
          Platform.runLater(
              () -> {
                ...
                String finalLicensePath =
                    arPropertyManager.getProperty(ARPropertyEnum.PATH_LICENSE);
      ...
      // Ensure launch(args) is only called once: JavaFX does not allow calling Application.launch()
      // twice.
      launch();
    }
  }
  ...
  private static void licenseMessages(LicenceVal licenseStatus) {
    ...
    performMessage.showCustomModalDialogDragWin11(
        ...
        "<span style='color: #E65100; font-weight: bold;'>Current license status:</span> <span style='font-weight: bold;'>"
            ...
            + "</span>",
        "Expiration: <span style='color: "
            + msgColorExp
            + "; font-weight: bold;'>"
            + arPropertyManager.getProperty(ARPropertyEnum.EXPIRATION)
            + "</span>",
        false,
        "OK",
        null,
