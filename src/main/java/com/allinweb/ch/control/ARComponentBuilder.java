...
public class ARComponentBuilder {
  ...
  public AnchorPane createContentPanel(
      Double topPanelHeight, Double bottomPanelHeight, Double edgeSpace) {
  ...
  public <T extends Node> T setAnchorPaneAnchors(
      T node, Double top, Double bottom, Double left, Double right) {
  ...
  public ImageView buildImageView(String source, Double size) {
    try {
      ...
      ARLogger.getInstance(ARWebDriver.class)
          .severe(
              String.format(
                  "BuildImageView source: %s size %d \n%s", source, size, e.getMessage()));
    }
    return null;
  }
  ...
  public Button buildButton(
      String text, Double height, String iconSource, Double iconSize, Insets padding) {
