...
public class ARWebSocketServerIP {
  ...
  public static ARWebSocketServerIP getInstance() throws Exception {
    if (instance == null) {
  ...
  static {
    logger = ARLogger.getInstance(ARWebSocketServerIP.class);
    arPropertyManager = ARPropertyManager.getInstance();
    webSocketSessionManager = WebSocketSessionManager.getInstance();
  }

  /**
   * Starts the Jetty WebSocket server. Determines the port and bind address, then initializes and
   * starts Jetty.
   *
  ...
  public void startServer() throws Exception {
      ...
      if (fallbackPortStr != null) {
        try {
          ...
          if (isPortInUse(fallbackPort)) {
            logger.severe(
                "Fallback port " + fallbackPort + " is also in use. Cannot start server.");
            throw new IOException(
                "Cannot start server: Both initial and fallback ports are in use.");
          }
          this.boundPort = fallbackPort;
          logger.info("Using fallback port: " + this.boundPort);
        } catch (NumberFormatException e) {
          logger.severe(
              "Invalid port number in properties: " + fallbackPortStr + " " + e.getMessage());
        ...
        throw new IOException(
            "Cannot start server: Initial port "
                + initialPort
                + " is in use and no fallback defined.");
      }
    }
    ...
    jettyServer.start();
    logger.info(
        "Jetty WebSocket server started on ws://"
            + BIND_IP_ADDRESS
            + ":"
            + this.boundPort
            + "/websocket");
    logger.info(
        "Current active WebSocket sessions: " + webSocketSessionManager.getAllSessions().size());
  }
  ...
  /**
   * Determines the initial port for the server. First tries to get any available ephemeral port and
   * persists it to properties. If finding an ephemeral port fails, it falls back to a default fixed
   * port. The chosen port is always persisted to properties for consistency across runs.
   */
  private int getInitialPort() {
    ...
    try (ServerSocket tempSocket = new ServerSocket(0)) {
      ...
      logger.warning(
          "Could not find an ephemeral port. Falling back to default fixed port: "
              + defaultFixedPort
              + ". Error: "
              + e.getMessage());
    ...
    arPropertyManager.setProperty(
        ARPropertyEnum.PORT_SOCKET.getValue(), String.valueOf(chosenPort));
    logger.info(
        "Set " + ARPropertyEnum.PORT_SOCKET.getValue() + " to: " + chosenPort + " in properties.");
    ...
    try (ServerSocket serverSocket = new ServerSocket(port)) {
      ...
      if (message != null
          && (message.contains("Address already in use")
              || message.contains("socket bind failed"))) {
