...
public class SimpleWebSocketServer {
  ...
  public void onOpen(Session session) {
    ...
    if (sessionId != null) {
      webSocketSessionManager.addSession(
          sessionId, session); // Store the session with the custom ID
  ...
  private void handleMessageByType(
      String type, JsonObject jsonEntry, Session session, String sessionId) {
    ...
    switch (type) {
      case "CLOSE_BROWSER":
        ...
        homeBankingId =
            elementSplitDTO.getHomeBankingId() != null ? elementSplitDTO.getHomeBankingId() : -1;
      ...
      case "SEARCH_TOOL":
        ...
        if (sessionIdToSend.equals("scannerGrid")) {
          String jsonData = gson.toJson(elementSplitDTO);
          webSocketSessionManager.sendMessageJson(
              homeBankingId, sessionIdToSend, jsonData, "addPickOne");
          ...
          performMessage.outputJsonElementDTO(
              elementSplitDTO.getDetails(), excludeList, "elementDTO");
          excludeList =
              List.of(
                  "optional",
                  "blockMarked",
                  ...
                  "attributeValue");
          performMessage.outputJsonElementDTO(
              elementSplitDTO.getDetails(), excludeList, "AI-ElementDTO");
        }
          ...
          if ((sessionIdToSend != null && sessionIdToSend.matches(".*botJobTasks.*"))) {
            if (performDataBase.rowsUpdateName(rowUpdateDTO.getUpdatedRows())) {
              List<ParentOperations> listParents =
                  performDataBase.loadAllParents(
      ...
      case "ROW_MOVE":
        ...
        if ((sessionIdToSend != null && sessionIdToSend.matches(".*botJobTasks.*"))) {
          ...
          if (performDataBase.updateMoveRowsOrder(rowMoveDTO.getUpdatedRows())
            ...
            performDataBase.deleteBlockDirect(
                rowMoveDTO.getBotJobId(), rowMoveDTO.getDeleteBlockId());
          ...
          if (performDataBase.updateCompMoveRowsOrder(rowMoveDTO.getUpdatedRows())
            ...
            performDataBase.deleteCompBlockDirect(
                rowMoveDTO.getBotJobId(), rowMoveDTO.getDeleteBlockId());
        ...
        if (blockReorder.getUpdatedBlocks().size() > 0) {
          ...
          if ((sessionIdToSend != null && sessionIdToSend.matches(".*botJobTasks.*"))) {
            performDataBase.updateBlockOrderNumber(
                ...
                true);
            performDataBase.deleteNullBlocks(blockReorder.getUpdatedBlocks().get(0).getBotJobId());
      ...
      case "BLOCK_UPDATE":
        ...
        if ((sessionIdToSend != null && sessionIdToSend.matches(".*botJobTasks.*"))) {
          performDataBase.updateBlockName(
              blockUpdateDTO.getBotJobId(),
              blockUpdateDTO.getBlockId(),
      ...
      case "DELETE_INSTRUCTION":
        InstructionLoadDTO deleteInstructionLoadDTO =
            gson.fromJson(jsonEntry, InstructionLoadDTO.class);
        ...
        if ((sessionIdToSend != null && sessionIdToSend.matches(".*botJobTasks.*"))) {
          performDataBase.deleteInstruction(
              deleteInstructionLoadDTO.getBotJobId(), deleteInstructionLoadDTO);
        } else if ((sessionIdToSend != null && sessionIdToSend.matches(".*componentTasks.*"))) {
    ...
    if (!alreadySentMgsSocket
        && (sessionIdToSend != null && sessionIdToSend.matches(".*botJobTasks.*"))) {
      ...
      webSocketSessionManager.sendMessageJson(
          homeBankingId, sessionIdToSend, jsonData, "updateInstructions");

    } else if (!alreadySentMgsSocket
        && (sessionIdToSend != null && sessionIdToSend.matches(".*componentTasks.*"))) {
      this.botJobLoadList =
          performDataBase.loadComponentsComplete(homeBankingId, botJobIdTask, botJobNameTask);
      String jsonData = "[]";
      if (!botJobLoadList.isEmpty()) {
        List<InstructionLoadDTO> blockLoopInstructions =
      ...
      webSocketSessionManager.sendMessageJson(
          homeBankingId, "componentTasks", jsonData, "componentsUpdate");

      //            broadcastMessageToAll(homeBankingId, "componentTasks", jsonData,
      // "componentsUpdate");
  ...
  public void onClose(Session session, CloseReason closeReason) {
    ...
    if (sessionId != null) {
      ...
      System.out.println(
          "Connection closed for unknown session, Reason: "
              + closeReason.getReasonPhrase()
              + " (Code: "
              + closeReason.getCloseCode()
              + ")");
  ...
  private void injectStepAfterOrBefore(String sessionId, RowMoveDTO rowMoveDTO) {
    ...
    if (rowMoveDTO.getUpdatedRows().size() > 0) {
      ...
      // performDataBase.loadBotJobById(rowMoveDTO.getBotJobId());
      ...
      if (!rowMoveDTO.getType().equals("INSERT_BEFORE_ELSEIF")
        ...
        //                Platform.runLater(() -> {
        //                    arNewCommandScene.initialize(rowMoveDTO, botJobLoad,
        // this.webPageItems,
        // sessionId);
  ...
  private void injectBlockComponent(BlockSplitDTO blockSplitDTO) {
    ...
    if (errorMessage == null) {
      List<BotJobLoadDTO> botJobLoadList =
          performDataBase.loadCompleteJobs(blockDetailsDTO.getBotJobId());
