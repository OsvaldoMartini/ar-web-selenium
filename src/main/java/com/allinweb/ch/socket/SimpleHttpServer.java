...
public class SimpleHttpServer {
  ...
  public static int start(int requestedPort, String filePath) throws Exception {
    ...
    try {
      ...
      if (resourceUrl == null) {
        logger.severe(
            "Resource not found for path: " + filePath + ". Please ensure it's on the classpath.");
      ...
      logger.severe(
          "Failed to resolve index.html path for resource '"
              + filePath
              + "': "
              + error.getMessage());
    ...
    if (!Files.exists(indexHtmlFilePath)) {
      ...
      throw new IOException(
          "index.html file not found at resolved path: " + indexHtmlFilePath.toAbsolutePath());
    }
    ...
    if (baseDirectory
        == null) { // Handle case where index.html is in the root of a drive/filesystem
      baseDirectory = Paths.get("./"); // Use current directory as fallback base
      logger.warning(
          "Index.html has no parent directory. Using current directory as base: "
