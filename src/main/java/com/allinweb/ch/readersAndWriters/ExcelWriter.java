...
public class ExcelWriter {
  ...
  private static final DateTimeFormatter FORMAT_DATE_AND_TIME =
  ...
  public ExcelWriter(String botJobName, WebDriver webDriver, boolean isFullPath) {
    ...
    try {
      ...
      managedExcelMap.put(
          "report", new ManagedExcel(botJobName + " (" + now + ")", "report", true, isFullPath));
    } catch (Exception ex) {
  ...
  public record ExcelChain(ManagedExcel managedExcel, String botJobName) {
    ...
    public static void writeMapToCSV(
        Map<String, String> mapExport, String filePath, String delimiterCSV) {
    ...
    public boolean insertInstructionResult(
      ...
      try {
        ...
        String[] operations =
            !Strings.isNullOrEmpty(msgLoop.getValue())
        ...
        if (actions[0].equalsIgnoreCase(ARConstants.NEXT_ROW)) {
          ...
          if (msgLoop.getValue().equals("Unknown")) {
            ...
            keyAction =
                String.format(
                    "Jump To Parent \"%s\"",
                    msgParent[0] + "-(" + msgParent[1] + ") " + msgParent[2]);
          ...
          keyAction =
              String.format(
                  "Wait for \"%s\"", msgParent[0] + "-(" + msgParent[1] + ") " + msgParent[2]);
        ...
        boolean byPassError =
            currentCondition.equals(ARConstants.ConditionStatus.IF_FAILED)
        ...
        ARLogger.getInstance(ExcelWriter.class)
            .severe(
                String.format(
                    "InsertInstructionResult ( %s ) Error: %s ",
                    msgLoop.getKey(), ex.getMessage()));
        return false;
      }
    }
    ...
    public void insertTotalExecutionTimes(long startTime, long endTime) {
      ...
      managedExcel
          ...
          .insertValueAtCoordinates(
              ...
              3,
              EXECUTION_TIMES_COLUMN_INDEX + 1)
    ...
    public ManagedExcel(String fileName, String purpose, boolean create, boolean isFullPath) {
      ARPropertyEnum property =
          switch (purpose) {
            ...
            default ->
                throw new UnsupportedOperationException("Purpose: " + purpose + " not supported");
          };
      String fileNamePath = "";
      String fullPath = "";
  ...
  record ManagedExcelAction(Sheet sheet, XSSFWorkbook workbook) {
    ...
    public ManagedExcelAction insertValueAtCoordinates(
        String value, int rowIndex, int columnIndex) {
    ...
    public ManagedExcelAction insertValueAfterLastRowOfColumn(
        String value, int exportIndex, int columnIndex) {
    ...
    public ManagedExcelAction insertImageAtCoordinates(
        String value, int rowIndex, int columnIndex) {
      // TODO: to implement if needed
      return this;
    }

    public ManagedExcelAction insertScreenshotAfterLastRowOfColumn(
        int columnIndex, WebDriver webDriver) {
      int afterLastRowIndex = sheet.getLastRowNum() + 1;
    ...
    public ManagedExcelAction insertScreenshotAtCoordinates(
        int rowIndex, int columnIndex, WebDriver driver) {
      try {
