...
public class ExcelUtils {
  ...
  private void generateUnfilteredCSVFile(BotJobDTO botJob) {
    ...
    try {
      ...
      for (BlockLoadDTO block : lisBotJobBlocks.get(0).getBlockLoadDTOList()) {
        ...
        //                                (instruction) ->
        // instruction.getBlockId().equals(block.getId())
        //                                        &&
        ...
        List<InstructionLoadDTO> allInstructions =
            performDataBase.getInstructionsByBlockId(
                block.getBotJobId(), block.getId(), "instruction");
  ...
  private File generateUnfilteredExcelFile(
    ...
    if (blockList.size() > 0) {
      ...
      for (BlockLoadDTO block : blockList) {
        ...
        //                List<InstructionLoadDTO> filteredInstructions =
        // InstructionLoadDTO.stream()
        ...
        for (InstructionLoadDTO instruction : filteredInstructions) {
          ...
          if (hasReference) {
            ...
            if (!fieldAddedSet.contains(reference)) {
              ...
              Cell instructionFieldCell =
                  instructionFieldRow.createCell(currentIndex, CellType.STRING);
  ...
  public static ExtractedData isFileExists(String botJobName, List<String> allActions) {
    ...
    String fileName =
        String.format("%s/%s%s", excelFolderPath, botJobName, ARConstants.FILE_FORMAT_EXCEL);
