...
public class ARPropertyManager {
  ...
  public void setProperty(String propertyName, String value) {
    ...
    try (FileOutputStream output = new FileOutputStream(configurationFileName)) {
      //            this.properties.store(output, "added property: " + propertyName + " with value:
      // " + value);
  ...
  public boolean missingMandatoryPats() {
    ...
    if (!missingProperties.isEmpty()) {
      ...
      String part1 =
          String.join(", ", missingProperties.subList(0, Math.min(partSize, totalMissing)));
      String part2 =
          totalMissing > partSize
              ? String.join(
                  ", ", missingProperties.subList(partSize, Math.min(2 * partSize, totalMissing)))
              : "";
      String part3 =
          totalMissing > 2 * partSize
