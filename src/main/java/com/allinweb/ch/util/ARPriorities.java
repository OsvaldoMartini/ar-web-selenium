  ...
  public void loadPriorities() {
    ...
    if (!prioritiesFile.exists()) {
      JOptionPane.showMessageDialog(
          ...
          "Priority configuration file is missing. Please check that the priority configuration is "
              + "set correctly or create the file: "
              + prioritiesFileName,
    ...
    try (FileInputStream priorities = new FileInputStream(prioritiesFile)) {
      ...
      if (properties.size() == 0) {
        ARLogger.getInstance(ARPriorities.class)
            .warning("The file " + prioritiesFileName + "is empty");
      }
      properties
          .keySet()
          .forEach(
              keyObj -> {
                ...
                Priority priority =
                    new Priority(
                        ...
                        params[1],
                        Arrays.stream(Arrays.copyOfRange(params, 2, params.length)).toList());
  ...
  public static void loadPrioritiesFromString(String text) {
    ...
    for (String line : lines) {
      ...
      if (parts.length >= 3) {
        ...
        List<String> name = Arrays.stream(Arrays.copyOfRange(parts, 2, parts.length)).toList();
