...
public class ARInfoPane extends ARPane {
  ...
  public void initUIComponents() {
    ...
    Label compileDateLabel =
        new Label("Build: " + arPropertyManager.getProperty(ARPropertyEnum.BUILD));
    ...
    btnLicense.setStyle(
        "-fx-background-color: #007bff; -fx-text-fill: white; -fx-font-size: 14px;"); // Blue color

    // Optional: Add a hover effect to change button color when hovered
    btnLicense.setOnMouseEntered(
        event ->
            btnLicense.setStyle(
                "-fx-background-color: #0056b3; -fx-text-fill: white; -fx-font-size: 14px;"));
    btnLicense.setOnMouseExited(
        event ->
            btnLicense.setStyle(
