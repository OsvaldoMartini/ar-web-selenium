...
public class ARNewBotJobPane extends ARPane {
  ...
  public void initUIBehaviour() {
    homeBankingChoiceBox.setConverter(
        new StringConverter<>() {
          @Override
  ...
  private void createBotJob() {
    Platform.runLater(
        () -> {
          ...
          boolean existName =
              botJobList.stream()
                  .anyMatch(f -> f.getName().equalsIgnoreCase(botJobName.getText().trim()));
          ...
          if (newJobId > 0) {
            ...
            ARLogger.getInstance(ARNewBotJobPane.class)
                .finer("ARNewBotJobPane CurrentStage close()");
            Platform.runLater(
                () -> {
