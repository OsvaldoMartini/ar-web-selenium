...
public class ARElementValuePane extends ARPane {
  ...
  public void initUIComponents() {
    ...
    comboBoxLocalFormat
        .getItems()
        .addAll(
            new FormatOption("American (9,999.99)", "US"),
            new FormatOption("European (9.999,99)", "EU"));

    comboBoxLocalFormat.setCellFactory(
        param ->
            new ListCell<>() {
              @Override
    ...
    comboBoxLocalFormat.setButtonCell(
        new ListCell<>() {
          @Override
          protected void updateItem(FormatOption item, boolean empty) {
            super.updateItem(item, empty);
    ...
    comboBoxCSVColumns.setButtonCell(
        new ListCell<>() {
          @Override
          protected void updateItem(FormatOption item, boolean empty) {
            super.updateItem(item, empty);
    ...
    comboBoxCSVColumns.setCellFactory(
        param ->
            new ListCell<>() {
              @Override
              protected void updateItem(FormatOption item, boolean empty) {
    ...
    submitButton.setOnAction(
        event -> {
          String selectedType =
              stringCheckBox.isSelected()
                  ? "$String"
                  : numericCheckBox.isSelected() ? "#Numeric" : "";

          String valueVar =
              Strings.isNullOrEmpty(valueField.getText()) ? "$EMPTY" : valueField.getText();
          ...
          if (nameExists(nameField.getText().trim())) {
            performMessage.errorMessage(
                ...
                String.format(
                    "'%s' cannot be inserted with the existent name!", nameField.getText()),
                null,
                null,
                null,
                0);
    ...
    HBox typeBox =
        new HBox(10, stringCheckBox, numericCheckBox); // Create an HBox to hold the checkboxes
  ...
  private void fillFields(VariableUserDTO userDTO) {
    ...
    if (!Strings.isNullOrEmpty(userDTO.getLocalFormat()) && userDTO.getLocalFormat().equals("EU")) {
    ...
    // Update the checkboxes based on the selected user's type
    if (!Strings.isNullOrEmpty(userDTO.getDelimiter()) && userDTO.getDelimiter().equals("|")) {
  ...
  private <S, T> void applyStyledColumn(
      TableColumn<S, T> column, Callback<T, String> renderFunction) {
    column.setCellFactory(
        tc ->
            new TableCell<S, T>() {
              @Override
