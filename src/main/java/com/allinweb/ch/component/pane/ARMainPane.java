...
public class ARMainPane extends ARPane {
  ...
  @Getter private ObservableList<WebDriver> webDriverList;
  ...
  public void initUIComponents() {
    ...
    viewBotJobListView.setCellFactory(
        new ARCellFactory<>(
                BotJobListCell.class, arViewBotJobScene, arWebDriver, botJobList, webDriverList)
            ::call);
  ...
  public void initUIBehaviour() {
    ...
    newBotJobButton.setOnMouseClicked(
        e -> {
          arNewBotJobScene.initialize(arViewBotJobScene, arWebDriver, botJobList, webDriverList);
    ...
    cloneBotJobButton.setOnMouseClicked(
        e -> {
          if (!checkLicense()) {
            return;
          }
          ...
          if (selecBotJobDTO != null) {
            ...
            performMessage.errorMessage(
                "Select a Bot Job", "There is NOT a Job Selected", null, null, null, 0);
            return;
          }
        });

    configureButton.setOnMouseClicked(
        e -> {
    ...
    editBotJobButton.setOnMouseClicked(
        e -> {
          if (!checkLicense()) {
            return;
          }

          BotJobLoadDTO selecBotJobDTO = viewBotJobListView.getSelectionModel().getSelectedItem();
    ...
    launchBotJobButton.setOnMouseClicked(
        e -> {
          ...
          if (selecBotJobDTO != null) {
            String enginePath =
                arPropertyManager.getProperty(
                    ARPropertyEnum.PATH_ENGINE); // + "\\AR_Web_Engine.jar";
