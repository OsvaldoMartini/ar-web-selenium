...
public class ARExcelFilePane extends ARPane {
  ...
  public void initUIComponents() {
    ...
    titleLabel =
        new Label(
            "#"
                ...
                + this.blockExcelDTO.getBlockName());
    titleLabel.setStyle(
        "-fx-font-size: 18px; -fx-font-weight: bold; -fx-text-fill: darkgreen;"); // Green dark
    ...
    String excelPath =
        blockExcelDTO.getExportFile() != null && !blockExcelDTO.getExportFile().isEmpty()
    ...
    if (!excelPath.isEmpty()) {
      try {
        ...
        directory =
            path.getParent() != null ? path.getParent().toString() : ""; // Get the directory path
        fileName =
            path.getFileName() != null ? path.getFileName().toString() : ""; // Get the file name
        ...
        ARLogger.getInstance(ARExcelFilePane.class)
            .severe("Excel Path  \nError: " + ex.getMessage());
      }
  ...
  public void initUIBehaviour() {
    ...
    pathDeleteButton.setOnMouseClicked(
        e -> {
          pathExport.setText("");
          fileExport.setText("");
          saveConfigurations();
  ...
  private void saveConfigurations() {
    ...
    if (Strings.isNullOrEmpty(pathExport.getText())) {
      ...
      if (Strings.isNullOrEmpty(fileName)) {
        performMessage.errorMessage(
            "File Name  Is Empty!", "Type  the File Name!", null, null, null, 0);
    ...
    boolean updateBlock =
        performDataBase.updateBlockExportFile(
    ...
    if ((sessionId != null && sessionId.matches(".*botJobTasks.*"))) {
      ...
      botJobLoadList =
          performDataBase.loadComponentsComplete(
              blockExcelDTO.getHomeBankingId(),
              blockExcelDTO.getBotJobId(),
              blockExcelDTO.getBotJobName());
      String jsonData = "[]";
      if (!botJobLoadList.isEmpty()) {
